...
.. spicy:method:: bytes::split bytes split False vector<bytes> ([ sep: bytes ])
    ...
    returned vector will have the whole bytes value as its single element.
    If the separator is not given, the split will occur at sequences of
    white spaces.
...
.. spicy:method:: bytes::split1 bytes split1 False tuple<bytes,~bytes> ([ sep: bytes ])
    ...
    element and the whole bytes value as its second element. If the
