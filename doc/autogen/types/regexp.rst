...
.. spicy:method:: regexp::find regexp find False tuple<int<32>,~bytes> (data: bytes)
    ...
    positions. Returns a 2-tuple with (1) a integer match indicator with
...
.. spicy:method:: regexp::match regexp match False int<32> (data: bytes)
    ...
    wouldn't change anything. The expression is considered anchored to the
...
.. spicy:method:: regexp::match_groups regexp match_groups False vector<bytes> (data: bytes)
    ...
    to the beginning of the data. This method is not compatible with
    pattern sets and will throw a runtime exception if used with a regular
...
.. spicy:method:: regexp::token_matcher regexp token_matcher False hilti::MatchState ()
    ...
    against chunks of future input. The expression will be considered
    anchored to the beginning of any data.
