...
static std::string _fmtAlt(const codegen::Production& alt, const std::set<codegen::Production>& lahs) {
    auto fmt = [&](const auto& lah) {
        if ( lah.isLiteral() )
            return util::fmt("%s (id %" PRId64 ")", lah.render(), lah.tokenID());

        return util::fmt("%s (not a literal)", lah.render());
    };

    return util::fmt("{%s}: %s", util::join(util::transform(lahs, fmt), ", "), alt.symbol());
}
