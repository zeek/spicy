        ...
        case TypeInfo::Struct: {
            ...
            for ( const auto& [f, y] : struct_->iterate(v, false, true) ) {
                if ( f.type->tag == TypeInfo::Bitfield && f.isAnonymous() ) {
                    ...
                    if ( f.isEmitted() && y ) {
                        ...
                        for ( const auto& b : f.type->bitfield->bits() )
                            j[b.name] =
                                json(); // TODO: This the right choice to use? Skip non-emitted fields altogether?
                    }
                ...
                if ( f.isEmitted() && y )
                    ...
                    j[f.name] = json(); // TODO: This the right choice to use? Skip non-emitted fields altogether?
            }
