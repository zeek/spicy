...
UnqualifiedType* CodeGen::compileUnit(type::Unit* unit, bool declare_only) {
    ...
    if ( options().getAuxOption<bool>("spicy.track_offsets", false) ) {
        ...
        v.addField(builder()->declarationField(ID("$offsets"), map,
    ...
    if ( auto* context = unit->contextType() ) {
        ...
        auto* f =
            builder()->declarationField(ID("$context"), builder()->qualifiedType(ftype, hilti::Constness::Mutable),
                                        attrs, unit->meta());
        v.addField(f);
    }
    ...
    add_hook("0x25_init", {});
    add_hook("0x25_done", {});
    add_hook("0x25_error", {builder()->parameter("$except", builder()->typeString())});
    add_hook("0x25_print", {});
    ...
    if ( auto id = unit->canonicalID() ) {
        ...
        addDeclaration(builder()->constant(ID(fmt("$feat%%%s%%uses_offset", type_id)), builder()->bool_(true)));
        addDeclaration(builder()->constant(ID(fmt("$feat%%%s%%uses_random_access", type_id)), builder()->bool_(true)));
        addDeclaration(builder()->constant(ID(fmt("$feat%%%s%%uses_stream", type_id)), builder()->bool_(true)));
        addDeclaration(builder()->constant(ID(fmt("$feat%%%s%%uses_sync_advance", type_id)), builder()->bool_(true)));
        addDeclaration(
            builder()->constant(ID(fmt("$feat%%%s%%is_filter", type_id)), builder()->bool_(unit->isFilter())));
        addDeclaration(builder()->constant(ID(fmt("$feat%%%s%%supports_filters", type_id)), builder()->bool_(true)));
        addDeclaration(builder()->constant(ID(fmt("$feat%%%s%%supports_sinks", type_id)), builder()->bool_(true)));
        addDeclaration(builder()->constant(ID(fmt("$feat%%%s%%synchronization", type_id)), builder()->bool_(true)));
    }
    ...
    auto* stream =
        builder()->declarationField(ID("$stream"),
    ...
    auto* f3 =
        builder()->declarationField(ID("$offset"),
...

    {
        ...
        auto* parser = builder()->declarationField(ID("$parser"),
...

    {
        ...
        auto* sink = builder()->declarationField(ID("$sink"),
    ...
    v.addField(
        builder()->declarationField(ID("$parse_stage1"), builder()->qualifiedType(ft, hilti::Constness::Mutable), {}));
    ...
    if ( auto* convert = unit->attributes()->find(attribute::kind::Convert) ) {
        ...
        auto* function = builder()->function(ID("$convert"), ftype, body->block());
        auto* convert_ = builder()->declarationField(ID("$convert"), function, {});
        v.addField(convert_);
    }
...
void CodeGen::compilePublicUnitAlias(hilti::declaration::Module* module, const ID& alias_id, type::Unit* unit) {
    ...
    auto* parser_field = builder()->declarationField(ID("$parser"),
                                                     builder()->qualifiedType(builder()->typeName("spicy_rt::Parser"),
                                                                              hilti::Constness::Mutable),
                                                     attrs);

    auto struct_id = ID(alias_id.namespace_(), "$parser_" + alias_id.local().str());
...
void CodeGen::_compileParserRegistration(const ID& public_id, const ID& struct_id, type::Unit* unit) {
    ...
    _pb.guardFeatureCode(unit, dependent_feature_flags, [&]() {
        ...
        _pb.builder()->addAssign(builder()->id(ID(struct_id, "$parser")), parser);

        _pb.builder()->addExpression(
            builder()->call("spicy_rt::registerParser",
                            {builder()->id(ID(struct_id, "$parser")), builder()->scope(),
    ...
    auto* register_unit =
        builder()->function(ID(fmt("$register_%s_%s", hiltiModule()->uid(), public_id.local())),
