...
struct VisitorPass2 : public visitor::MutatingPostOrder {
    ...
    void operator()(operator_::unit::Offset* n) final { replaceNode(n, builder()->member(n->op0(), ID("$offset"))); }

    void operator()(operator_::unit::Position* n) final {
        auto* begin = builder()->member(n->op0(), ID("$begin"));
        auto* offset = builder()->member(n->op0(), ID("$offset"));
    ...
    void operator()(operator_::unit::SetInput* n) final {
        auto* cur = builder()->member(n->op0(), ID("$position_update"));
    ...
    void operator()(operator_::unit::ContextConst* n) final {
        auto* x = builder()->member(n->op0(), ID("$context"));
        replaceNode(n, x);
    }
    ...
    void operator()(operator_::unit::Stream* n) final {
        replaceNode(n, builder()->deref(builder()->member(n->op0(), ID("$stream"))));
    }
    ...
    void operator()(statement::Stop* n) final {
        ...
        b->addAssign(builder()->id("$stop"), builder()->bool_(true), n->meta());
        b->addReturn(n->meta());
...
hilti::declaration::Function* CodeGen::compileHook(const type::Unit& unit, const ID& id, type::unit::item::Field* field,
    ...
    if ( type == declaration::hook::Type::ForEach ) {
        ...
        params.push_back(
            builder()->parameter("$dd", field->ddType()->type()->elementType()->type(), hilti::parameter::Kind::In));
        params.push_back(builder()->parameter("$stop", builder()->typeBool(), hilti::parameter::Kind::InOut));
    }
    else if ( type == declaration::hook::Type::Error ) {
        if ( params.empty() )
            params.push_back(builder()->parameter("$except", builder()->typeString(), hilti::parameter::Kind::In));
    }
    else if ( original_field_type ) {
        assert_field();

        params.push_back(builder()->parameter("$dd", field->itemType()->type(), hilti::parameter::Kind::In));
        ...
        if ( original_field_type->type()->isA<hilti::type::RegExp>() && ! is_container )
            params.push_back(
                builder()->parameter("$captures", builder()->typeName("hilti::Captures"), hilti::parameter::Kind::In));
    }
    ...
    if ( id.local().str() == "0x25_print" ) {
        ...
        hid = fmt("$on_%s%s", id.local(), postfix);
...
Expression* CodeGen::addGlobalConstant(Ctor* ctor) {
    ...
    return cache.getOrCreate(ctor->print(), [&, &uniquer = uniquer]() { // need to capture `u` explicitly with C++17
        auto id = uniquer.get(ID(fmt("$%s", type)));
