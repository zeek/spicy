        ...
        auto parseField = [&](const auto& unit_fields, size_t field_index, Expression* dst_field_nr,
            ...
            if ( track_field_sizes ) {
                ...
                for ( auto i = field_index; i < unit_fields.size(); i++ ) {
                    ...
                    else if ( unit_fields[i]->parseSize(builder()) ) // TODO: parseSize doesnt work for ctor and/or
                                                                     // integer productions, at least
                        fixed_size_productions.push_back(production);
                }
...
#if 0
                // We can compute the total size of all remaining fields.
                auto begin_production =
                    builder()->addTmp("old_offset", builder()->memberCall(builder()->begin(state().cur), "offset"));

                auto try_ = builder()->addTry();
                pushBuilder(try_.first, [&]() { parseProduction(*production); });
                pushBuilder(try_.second.addCatch(builder()->parameter(ID("e"),
                                                                      builder()->typeName("hilti::RecoverableFailure"),
                                                                      hilti::parameter::Kind::InOut)),
                            [&]() {
                                // Compute the size of the remaining fields and
                                // then turn the error into one that knows
                                // where the current production (i.e., the
                                // unit) ends.
                                //
                                // Note that we *always* do this, because we
                                // cannot know if our higher-level caller will
                                // want to make use of this information or not
                                // (i.e., has an appropriate `&synchronize`).
                                Expression* total_size = nullptr;

                                for ( auto* p : fixed_size_productions ) {
                                    Expression* production_size = nullptr;

                                    if ( auto* field = p->meta().field();
                                         field && field->attributes()->find(attribute::kind::Size) )
                                        production_size = pb->evaluateAttributeExpression(field->attributes()->find(
                                                                                              attribute::kind::Size),
                                                                                          "size");
                                    else
                                        production_size = p->parseSize(builder());

                                    if ( total_size )
                                        total_size = builder()->sum(total_size, production_size);
                                    else
                                        total_size = production_size;
                                }

                                auto end_offset = builder()->sum(begin_production, total_size);
                                builder()->addCall("hilti::set_skip_offset", {builder()->id("e"), end_offset});
                                builder()->addRethrow();
                            });
            }
            else
        ...
        pushBuilder(try_.second.addCatch(builder()->parameter(ID("e"), builder()->typeName("hilti::RecoverableFailure"),
                    ...
                    [&]() {
                        ...
                        for ( auto x : hilti::util::enumerate(p->fields()) ) {
                            ...
                            auto* field = field_production->meta().field();
                            ...
                            pushBuilder(std::move(true_), [&]() {
                                if ( field && field->attributes()->find(attribute::kind::Size) )
                                    builder()->addAssign(field_offset, builder()->sum(builder()->deref(field_offset),
                                                                                      pb->evaluateAttributeExpression(
                                                                                          field->attributes()->find(
                                                                                              attribute::kind::Size),
                                                                                          "size")));
                                else if ( auto* size = field_production->parseSize(builder()) )
