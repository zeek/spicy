...
struct ProductionVisitor
    ...
    void syncProduction(const Production& p_) {
        ...
        if ( unitType ) {
            ...
            auto synchronizeFunction = [&](const hilti::expression::ResolvedID& fn) {
                pushBuilder(builder()->addWhile(builder::bool_(true)), [&]() {
                    auto found = builder()->addTmp("found", type::SignedInteger(8));
                    auto num = builder()->addTmp("num", type::UnsignedInteger(64));
            ...
            if ( synchronize_at ) {
                ...
                else if ( auto fn = synchronize_at->expression()->tryAs<hilti::expression::ResolvedID>() )
