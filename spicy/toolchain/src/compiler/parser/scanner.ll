...
False                 yylval->emplace<bool>(0); return token::CBOOL;
True                  yylval->emplace<bool>(1); return token::CBOOL;
None                  return token::NONE;
Null                  return token::CNULL;

{attribute}           yylval->emplace<str>(yytext); return token::ATTRIBUTE;
<INITIAL>{property}   yylval->emplace<str>(yytext); return token::PROPERTY;
...
{id}(::{id}){1,}(::{property})?       yylval->emplace<str>(yytext); return token::SCOPED_IDENT;
{id}(::{property})?    yylval->emplace<str>(yytext); return token::SCOPED_IDENT;
\$[1-9][0-9]*          yylval->emplace<uint64_t>(hilti::util::chars_to_uint64(yytext + 1, 10, range_error_int)); return token::DOLLAR_NUMBER;
\${id}                 yylval->emplace<str>(yytext + 1); return token::DOLLAR_IDENT;
...
<HOOK_ID>%?{id}(\.{id})*  yylval->emplace<str>(yytext); return token::HOOK_IDENT;
<HOOK_ID>({id}::){1,}%?{id}(\.{id})*  yylval->emplace<str>(yytext); return token::HOOK_IDENT;
