...
class Set : public TypeBase,
            ...
            trait::isParameterized {
public:
    Set(Type t, Meta m = Meta()) : TypeBase({std::move(t)}, std::move(m)) {}
    Set(Wildcard /*unused*/, Meta m = Meta()) : TypeBase({node::none}, std::move(m)), _wildcard(true) {}
    ...
    /** Implements the `Type` interface. */
    Type iteratorType(bool const_) const { return set::Iterator(*this, const_, meta()); }
    /** Implements the `Type` interface. */
    auto isWildcard() const { return _wildcard; }
    /** Implements the `Type` interface. */
    auto typeParameters() const { return childs(); }
