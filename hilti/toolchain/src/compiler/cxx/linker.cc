...
void cxx::Linker::finalize() {
    ...
    unit.add(fmt("const char HILTI_EXPORT HILTI_WEAK * %s_library_version = R\"(%s)\";", cxx_namespace, version.toJSON()));
    unit.add(fmt("const char HILTI_EXPORT HILTI_WEAK * %s_bind_to_version = " HILTI_VERSION_FUNCTION_STRING "();", cxx_namespace));
