    ...
    if ( add_ctors ) {
        ...
        if ( ! locals_user.empty() ) {
            auto locals_ctor_args = util::join(locals_user | std::views::transform([&](const auto& x) {
                                               ...
                                               ", ");
            auto locals_ctor = fmt("explicit %s(::hilti::rt::struct_::Args, %s);", type_name, locals_ctor_args);
        ...
        if ( args.size() ) {
            ...
            auto params_ctor_args =
                util::join(args | std::views::transform([&](const auto& x) { return fmt("%s %s", x.type, x.id); }),
                           ", ");
            auto params_ctor = fmt("%s(::hilti::rt::struct_::Args, %s);", type_name, params_ctor_args);
...
std::string cxx::type::Struct::code() const {
    ...
    if ( args.size() ) {
        ...
        if ( ctor_args.empty() )
            ...
            code += fmt("%s::%s(::hilti::rt::struct_::Args, %s) : %s {\n%s%s}\n\n", type_name, type_name, ctor_args,
