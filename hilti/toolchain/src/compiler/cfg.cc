...
static bool _isAliasingType(const UnqualifiedType& type) {
    ...
    if ( type.isA<type::stream::View>() ||     //
         ...
         type.isA<type::StrongReference>() ||  //
         type.isA<type::WeakReference>() ||    //
         type.isA<type::ValueReference>() )
        return true;
...
CFG::CFG(const Node* root)
    : _begin(_getOrAddNode(createMetaNode<Start>())), _end(_getOrAddNode(createMetaNode<End>(root))) {
...
GraphNode CFG::_addBlock(GraphNode predecessor, const Nodes& stmts, const Node* scope) {
    ...
    auto scope_end = _getOrAddNode(createMetaNode<End>(scope));
    ...
    if ( has_dead_flow && last != stmts.end() ) {
        ...
        auto mix = _getOrAddNode(createMetaNode<Flow>());
...
GraphNode CFG::_addFor(GraphNode predecessor, const statement::For& for_) {
    ...
    auto scope_end = _getOrAddNode(createMetaNode<End>(&for_));
...
GraphNode CFG::_addTryCatch(GraphNode predecessor, const statement::Try& try_catch) {
    ...
    auto mix_after = _getOrAddNode(createMetaNode<Flow>());
    _addEdge(try_, mix_after);

    // Since the `try` block can throw connect into node flowing into all `catch` blocks.
    auto mix_into_catches = _getOrAddNode(createMetaNode<Flow>());
...
void CFG::_populateDataflow() {
    ...
    do {
        ...
        for ( auto& [n, transfer] : _dataflow ) {
            ...
            // Populate the in set.
            std::unordered_map<Node*, std::unordered_set<GraphNode>> new_in;
            for ( auto& pid : g.neighborsUpstream(*id) ) {
                ...
                for ( const auto& [decl, stmts] : _dataflow.at(*p).out ) {
            ...
            std::unordered_map<Node*, std::unordered_set<GraphNode>> new_out;
