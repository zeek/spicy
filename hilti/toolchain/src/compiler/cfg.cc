...
#include "ast/operators/function.h"
...
CFG::NodeP CFG::add_block(NodeP parent, const Nodes& stmts) {
    auto* scope = parent ? parent->getData() : nullptr;
    while ( scope && ! scope->isA<statement::Block>() )
        scope = scope->parent();
    if ( scope && ! scope->isA<statement::Block>() )
        scope = nullptr;
    ...
    for ( auto&& c : (last != stmts.end() ? Nodes(stmts.begin(), last) : stmts) ) {
        ...
        if ( auto&& while_ = c->tryAs<statement::While>() )
            ...
            parent = add_block(parent, block->statements());
            ...
            auto x = add_block(parent, c->children());
    ...
    // When a block ends insert a dedicated meta node so we can prevent leaking of locals out of their block.
    if ( scope ) {
...
std::string CFG::dot() const {
    ...
    for ( auto&& n : sorted_nodes ) {
        ...
        if ( auto it = dataflow.find(n.get()); it != dataflow.end() ) {
            ...
            auto use = [&]() {
                auto xs = util::transformToVector(transfer.use, [](auto* decl) {
                    return rt::escapeUTF8(decl->template as<const hilti::Declaration>()->id());
                });
            ...
            auto gen = [&]() {
                auto xs = util::transformToVector(transfer.gen, [](auto&& kv) {
                    auto&& [decl, node] = kv;
                    return util::fmt("%s: %s", rt::escapeUTF8(decl->template as<const hilti::Declaration>()->id()),
                                     rt::escapeUTF8(node->getData()->print()));
                });
                std::sort(xs.begin(), xs.end());
                if ( ! xs.empty() )
                                     ...
                                     util::join(util::filter(
                                                    [&]() {
                                                        auto xs = util::transformToVector(nodes, [](auto&& x) {
                                                            return rt::escapeUTF8(x->getData()->print());
                                                        });
            ...
            auto reachability = [&]() -> std::string {
                ...
                auto to_str = [](auto&& xs) {
                    auto ys =
                        util::transformToVector(xs, [](auto&& x) { return rt::escapeUTF8(x->getData()->print()); });
        ...
        if ( auto&& meta = data->tryAs<MetaNode>() ) {
            ...
            ss << util::fmt("\t%s [label=\"%s\" %s];\n", id, rt::escapeUTF8(data->print()), xlabel ? *xlabel : "");
        }
    }
        ...
        for ( const auto& n : nodes ) {
            ...
            for ( const auto& e : inEdges(g, n.get()) ) {
                ...
                for ( auto&& f : from_ ) {
                    ...
                    if ( scope_end ) {
                        auto is_local =
                            std::find_if(dataflow.begin(), dataflow.end(), [&](const auto& flow) {
                                auto&& [_, transfer_] = flow;
                                return std::any_of(transfer_.gen.begin(), transfer_.gen.end(), [&](auto&& g) {
                                    auto&& [decl, use] = g;
                                    return use == f && contains(*scope_end->scope, *decl);
                                });
                            }) != dataflow.end();
                        if ( is_local ) {
                            continue;
            ...
            // Fix up use information for function calls. Since we currently do
            // not know whether a function makes use of globals mark all
            // globals as used.
            //
            // FIXME(bbannier): Remove is this once we can detect that a function is pure.
            if ( auto* stmt = n->getData()->tryAs<statement::Expression>() ) {
                if ( auto* call = stmt->expression();
                     call && (call->isA<operator_::function::Call>() || call->isA<operator_::struct_::MemberCall>()) ) {
                    // Mark all predecessors as used.
                    for ( auto* i : in ) {
                        assert(dataflow.count(n.get()));
                        auto [_, inserted] = dataflow[n.get()].use.insert(i->getData());
                        changed |= inserted;
                    }
                }
            }
...
std::vector<const CXXGraph::Node<CFG::N>*> CFG::unreachable_statements() const {
    ...
    // FIXME(bbannier): sorting is only for debugging, remove.
    std::sort(result.begin(), result.end(),
              [](const auto& a, const auto& b) { return a->getUserId() < b->getUserId(); });
