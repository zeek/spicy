...
Node* CFG::add_globals(Node* parent, const Node& root) {
    auto* p = root.parent();
    if ( ! p )
        return parent;
...
Node* CFG::add_block(Node* parent, const Nodes& stmts, const Node& scope) {
    ...
    for ( auto&& c : (last != stmts.end() ? Nodes(stmts.begin(), last) : stmts) ) {
        ...
        if ( auto&& while_ = c->tryAs<statement::While>() )
            ...
            parent = add_throw(parent, *throw_, scope_end);
...
Node* CFG::add_for(Node* parent, const statement::For& for_) {
    auto&& sequence = get_or_add_node(for_.sequence());
    add_edge(parent, sequence);

    auto&& local = get_or_add_node(for_.local());
    add_edge(sequence, local);

    auto* body_end = add_block(local, for_.body()->children(), *for_.body());
    add_edge(body_end, sequence);

    return sequence;
}

Node* CFG::add_while(Node* parent, const statement::While& while_) {
    if ( auto* init = while_.init() ) {
        auto* init_ = get_or_add_node(init);
        add_edge(parent, init_);
        parent = init_;
    }

    if ( auto* c = while_.condition() ) {
        auto&& condition = get_or_add_node(c);
        add_edge(parent, condition);
        parent = condition;
    }

    auto* body_end = add_block(parent, while_.body()->children(), *while_.body());
    add_edge(body_end, parent);

    auto* mix = get_or_add_node(create_meta_node<Flow>());
    add_edge(parent, mix);

    if ( auto&& else_ = while_.else_() ) {
        auto&& else_end = add_block(parent, else_->children(), *else_);
...
Node* CFG::add_if(Node* parent, const statement::If& if_) {
...
Node* CFG::add_try_catch(Node* parent, const statement::Try& try_catch) {
...
Node* CFG::add_return(Node* parent, const Node* expression) {
    if ( expression ) {
...
Node* CFG::add_throw(Node* parent, statement::Throw& throw_, Node* scope_end) {
    if ( auto* expression = throw_.expression() ) {
        auto* expr = get_or_add_node(expression);

        add_edge(parent, expr);
...
Node* CFG::add_call(Node* parent, operator_::function::Call* call) {
    auto* c = get_or_add_node(call);
    add_edge(parent, c);
    return c;
}

Node* CFG::get_or_add_node(Node* n) {
...
std::string CFG::dot() const {
    ...
    for ( auto&& n : sorted_nodes ) {
        ...
        if ( auto it = dataflow.find(n); it != dataflow.end() ) {
...
std::unordered_set<Node*> CFG::unreachable_nodes() const {
    ...
    { // Encode aliasing information.
        ...
        for ( auto&& [n, transfer] : dataflow ) {
            for ( const auto* alias : transfer.aliases ) {
                ...
                dataflow.at(stmt).aliases.insert(n);
            }
        }
...
void CFG::populate_reachable_expressions() {
    ...
    for ( auto&& [id, n] : nodes )
        dataflow.at(n).reachability = Reachability();
    ...
    while ( true ) {
        ...
        for ( const auto& [id, n] : nodes ) {
            ...
            auto& reachability = dataflow[n].reachability;
            ...
            for ( const auto& n : g.neighborsUpstream(n->identity()) ) {
                ...
                auto& from_ = dataflow.at(from).reachability->out; // Must already exist.
            ...
            const auto& gen = dataflow.at(n).gen;
            const auto& kill = dataflow.at(n).kill;
...
std::vector<Node*> CFG::unreachable_statements() const {
