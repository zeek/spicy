...
struct Visitor : hilti::visitor::PreOrder {
    ...
    void operator()(expression::Grouping* n) final {
        auto cxx_exprs =
            util::join(n->expressions() | std::views::transform([this](auto e) { return cg->compile(e); }), ", ");
        ...
        if ( auto* local = n->local() ) {
            ...
            f << block;
            result = fmt("([&](){%s return %s;}())", f.str(), cxx_exprs);
        }
        else
            result = fmt("(%s)", cxx_exprs);
    }
    ...
    void operator()(expression::Keyword* n) final {
        switch ( n->kind() ) {
            ...
            case expression::keyword::Kind::Scope: {
                auto scope = cxx::ID(fmt("%s_hlto_scope", cg->options().cxx_namespace_intern));
    ...
    void operator()(expression::Name* n) final {
        ...
        if ( auto* f = decl->tryAs<declaration::Field>() ) {
            if ( auto* fn = f->type()->type()->tryAs<type::Function>() ) {
                // If we're referring to, but not calling, a method, bind to the
                // type's fully-qualified name.
                switch ( fn->callingConvention() ) {
                    case type::function::CallingConvention::Extern: [[fallthrough]];
