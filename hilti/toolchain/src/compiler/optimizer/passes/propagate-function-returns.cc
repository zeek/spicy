...
struct CollectorPlacements : public optimizer::visitor::Collector {
    ...
    void collectFn(declaration::Function* n, bool revisiting) {
        ...
        for ( auto* use : *uses_of_op ) {
            if ( auto* ret = use->parent()->tryAs<statement::Return>(); ret && placements.tail_callee == nullptr ) {
                ...
                if ( ! fn_placements.contains(tail_caller_function_id) ||
                     fn_placements[tail_caller_function_id].placements.size() == 0 ) {
                    if ( ! revisiting )
                        revisit.insert(n);
                ...
                if ( tail_caller_placements.tail_callee && tail_caller_placements.tail_callee != n->function() ) {
                ...
                tail_caller_placements.tail_callee = n->function();
