...
struct CollectorUnusedParameters : public optimizer::visitor::Collector {
    ...
    std::map<ID, Placements> removable_params;
    ...
    void operator()(declaration::Function* n) final {
        ...
        // Create the removable placements
        if ( removable_params[function_id].placements.size() > 0 )
            return;
        ...
        auto& placements = removable_params.at(function_id);
        for ( auto* use : *uses_of_op ) {
            ...
            if ( auto* ret = use->parent()->tryAs<statement::Return>();
                ...
                if ( ! removable_params.contains(function_id) ||
                     removable_params[function_id].placements.size() == 0 ) {
                    revisit.push_back(n);
                    return;
                }

                auto& passthrough_placements = removable_params[function_id];
        ...
        if ( placements.passthrough_from ) {
            ...
            if ( ! fn_decl )
                return;
            auto& from_placements = removable_params[fn_decl->functionID(context())];
    ...
    void operator()(expression::Name* n) final {
        ...
        //       -> this name
        bool is_tup_ret =
            n->parent()->isA<ctor::Tuple>() && n->parent(2)->isA<expression::Ctor>() &&
            (n->parent(3)->isA<statement::Return>() || n->parent(3)->isA<operator_::struct_::MemberCall>());
        bool is_coerced_tup_ret =
            n->parent()->isA<ctor::Tuple>() && n->parent(2)->isA<ctor::Coerced>() &&
            n->parent(3)->isA<expression::Ctor>() &&
...
struct Mutator : public optimizer::visitor::Mutator {
    ...
    void operator()(declaration::Function* n) final {
        ...
        if ( ! collector_unused_parameters->removable_params.contains(function_id) )
            return;

        auto& placements = collector_unused_parameters->removable_params.at(function_id);
        ...
        if ( placements.passthrough_from ) {
            ...
            if ( ! fn_decl )
                return;
            placement_ids = collector_unused_parameters->removable_params[fn_decl->functionID(context())].placements;
        }
    ...
    std::optional<Node*> removeFromTupleCtor(ctor::Tuple* ctor, std::vector<std::optional<ID>> placements) {
        ...
        std::optional<Node*> replaced_with = nullptr;
        ;
        switch ( values.size() ) {
            case 0: break;
            case 1: replaced_with = values[0]; break;
            default: replaced_with = builder()->ctorTuple(values); break;
        }
        replaceNode(to_replace, *replaced_with);
