...
struct Mutator : public optimizer::visitor::Mutator {
    ...
    bool remove(detail::optimizer::CFG* cfg, Node* data, const std::string& msg = {}) {
        assert(data);
    ...
    void visitStatement(Statement* n, CFG* cfg) {
        while ( true ) {
            bool modified = false;
...
std::vector<Node*> Mutator::unusedStatements(const CFG* cfg) const {
    ...
    std::map<detail::optimizer::cfg::GraphNode, uint64_t> uses;
    ...
    for ( const auto& [n, transfer] : dataflow ) {
        ...
        if ( n->isA<detail::optimizer::cfg::End>() ) {
        ...
        if ( ! n->isA<detail::optimizer::cfg::MetaNode>() )
...
std::vector<Node*> Mutator::unreachableNodes(const CFG* cfg) const {
    std::unordered_set<Node*> tmp;
    for ( const auto& [id, n] : cfg->graph().nodes() ) {
        if ( n.value.value() && ! n.value->isA<detail::optimizer::cfg::MetaNode>() && n.neighbors_upstream.empty() )
