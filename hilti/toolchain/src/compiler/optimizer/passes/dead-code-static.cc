...
struct Collector : public optimizer::visitor::Collector {
    ...
    void operator()(expression::Member* n) final {
        ...
        const auto* struct_ = Optimizer::innermostType(op->op0()->type())->type()->tryAs<type::Struct>();
        if ( ! struct_ )
            return;
...
optimizer::RegisterPass constant_folder({.name = "dead-code-static",
                                         .order = 10,
                                         .post_processors = optimizer::PostProcessors::ScopeBuilder |
                                                            optimizer::PostProcessors::TypeUnifier |
                                                            optimizer::PostProcessors::Coercer |
                                                            optimizer::PostProcessors::CFG,
                                         .run = run});
