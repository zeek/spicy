...
struct Collector : optimizer::visitor::Collector {
    Collector(Optimizer* optimizer, Variables* variables)
        : optimizer::visitor::Collector(optimizer), variables(variables) {}
...
Function* containingFunction(Node* n) {
...
struct Mutator : optimizer::visitor::Mutator {
    ...
    void operator()(statement::Block* block) override {
        // Only work on blocks which are in other blocks. This excludes e.g.,
        // blocks which are function bodies, or bodies of `if` or `while`.
        auto* parent = block->parent()->tryAs<statement::Block>();
        if ( ! parent )
            return;

        // We only work on blocks in functions, but not global blocks.
        hilti::Function* function = containingFunction(block);
        if ( ! function )
            return;

        Variables variables;
        Collector(optimizer(), &variables).run(block);

        // Rename IDs which would clash with existing ones from the parent scope.

        auto& used = used_ids[function];

        for ( auto& [decl, uses] : variables ) {
            ID id = decl->id();
