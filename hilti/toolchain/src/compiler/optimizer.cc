    ...
    void operator()(declaration::Field* n) final {
        ...
        switch ( _stage ) {
            case Stage::COLLECT: {
                ...
                if ( n->attributes()->has(hilti::Attribute::Kind::ALWAYS_EMIT) )
                ...
                if ( auto type = type_ ) {
                    for ( const auto& requirement :
                          n->attributes()->findAll(hilti::Attribute::Kind::NEEDED_BY_FEATURE) ) {
    ...
    void operator()(declaration::Function* n) final {
        ...
        switch ( _stage ) {
            case Stage::COLLECT: {
                ...
                if ( fn->attributes()->has(hilti::Attribute::Kind::ALWAYS_EMIT) )
                ...
                if ( auto decl = context()->lookup(n->linkedDeclarationIndex()) ) {
                    for ( const auto& requirement :
                          fn->attributes()->findAll(hilti::Attribute::Kind::NEEDED_BY_FEATURE) ) {
                        const auto& requirement_ = requirement->valueAsString();
                        const auto& feature = *requirement_;
        ...
        switch ( _stage ) {
            case Stage::COLLECT: {
                ...
                for ( const auto& parameter : fn->function()->ftype()->parameters() ) {
                    ...
                    for ( const auto& requirement :
                          parameter->attributes()->findAll(hilti::Attribute::Kind::REQUIRES_TYPE_FEATURE) ) {
        ...
        switch ( _stage ) {
            case Stage::COLLECT: {
                ...
                if ( auto type_id = type->type()->typeID() )
                    for ( const auto& requirement :
                          field->attributes()->findAll(hilti::Attribute::Kind::NEEDED_BY_FEATURE) ) {
                ...
                for ( size_t i = 0; i < parameters.size(); ++i ) {
                    ...
                    if ( auto type_id = type->type()->typeID() )
                        for ( const auto& requirement :
                              param->attributes()->findAll(hilti::Attribute::Kind::REQUIRES_TYPE_FEATURE) ) {
                            const auto feature = *requirement->valueAsString();
                            if ( ! ignored_features.count(type_id) || ! ignored_features.at(type_id).count(feature) ) {
                                // Enable the required feature.
    ...
    void operator()(declaration::Type* n) final {
        switch ( _stage ) {
            case Stage::COLLECT: {
                ...
                for ( const auto& requirement :
                      n->attributes()->findAll(hilti::Attribute::Kind::REQUIRES_TYPE_FEATURE) )
                ...
                if ( ! _used.at(member_id) ) {
                    ...
                    if ( _features.count(type_id) ) {
                        ...
                        auto dependent_features =
                            hilti::node::transform(n->attributes()->findAll(hilti::Attribute::Kind::NEEDED_BY_FEATURE),
