...
struct VisitorPost : visitor::PreOrder, public validator::VisitorMixIn {
    ...
    void checkNodeAttributes(Node* n, node::Tag tag, AttributeSet* attributes, const std::string_view& where) {
        if ( ! attributes )
            return;
    ...
    void operator()(Function* n) final {
        checkNodeAttributes(n, n->nodeTag(), n->attributes(), "function");
    ...
    void operator()(declaration::Parameter* n) final {
        checkNodeAttributes(n, n->nodeTag(), n->attributes(), n->displayName());
        checkDeclarationType(n, n->type());
