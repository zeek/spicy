...
/**
 * Represents the kind of an attribute. While we generally identify attributes
 * by their names (`&...`), we wrap those strings into `Kind` instances so that
 * we can (1) define global constants for all valid attribute names, and (2)
 * retain a list of all known attributes for error checking. We use a class
 * instead managing attributes through a enum, so that the set of valid
 * attributes can be further extended elsewhere.
 */
class Kind {
public:
    Kind() = default;
    Kind(std::string name) : _name(std::move(name)) { _register(); }
    Kind(const Kind& other) = default;
    Kind(Kind&& other) = default;

    ~Kind() = default;

    auto to_string() const { return _name; }
    ...
    // exception.
    static Kind from_string(const std::string_view& s) {
...
inline auto to_string(const Kind& kind) { return kind.to_string(); }
...
namespace kind {
inline auto from_string(const std::string_view& s) { return Kind::from_string(s); }
...
struct hash<hilti::attribute::Kind> {
    size_t operator()(const hilti::attribute::Kind& x) const { return std::hash<std::string>()(x.to_string()); }
};
} // namespace std
