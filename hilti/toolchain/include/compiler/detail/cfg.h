...
struct Transfer {
    ...
    std::unordered_map<Node*, std::unordered_set<GraphNode>> in;

    /** Outgoing edges, ordered by declaration they work on. */
    std::unordered_map<Node*, std::unordered_set<GraphNode>> out;
    ...
    /** Values this node updates, ordered by declaration. */
...
public:
     ...
     * @param args contructor arguments for creating the `MetaNode` instance
     * @return a pointer to the added meta node
     */
    template<typename T, typename... Args>
    MetaNode* createMetaNode(Args... args)
    ...
    /** Get control flow. */
    const auto& graph() const { return g; }
...
private:
    ...
    util::graph::DirectedGraph<GraphNode, uint64_t> g;
