...
int main(int argc, char* argv[]) {
    ...
    if ( auto link_it = std::ranges::find_if(args, [](const auto& arg) { return arg == "-link" || arg == "/link"; });
         link_it != args.end() ) {
    ...
    if ( ! output_flag.empty() ) {
        ...
        auto it = std::ranges::find_if(args, [](const auto& arg) { return arg == "-link" || arg == "/link"; });
        args.insert(it, output_flag);
    }
