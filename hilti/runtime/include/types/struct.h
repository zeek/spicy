...
template<typename T>
constexpr auto has_hook_to_string_helper(int) -> decltype(std::declval<T>().$hook_to_string(), std::true_type{});
    ...
    requires(std::is_base_of_v<trait::isStruct, T>)
{
    if constexpr ( has_hook_to_string<T>() ) {
        if ( auto s = T(x).$hook_to_string() ) // copy because we need a non-const T
            return *s;
    }
