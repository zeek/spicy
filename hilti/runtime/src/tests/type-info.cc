...
TEST_CASE("traverse structs") {
    ...
    auto v = type_info::Value(&*sx, &$hlt::type_info::__ti_Test_X, p);
...
TEST_CASE("life-time") {
    ...
    auto v = type_info::Value(&*x, &$hlt::type_info::__ti_Test_X, p);
...
TEST_CASE("no parent") {
    ...
    auto x = StrongReference<Test::X>({42, "foo", y});
    auto p = type_info::value::Parent(x);
    auto v = type_info::Value(&*x, &$hlt::type_info::__ti_Test_X); // no parent
